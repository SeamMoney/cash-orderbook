...
const privyPackageInstalled = false
...
export default defineConfig(({ mode }) => {
  ...
  const overrides = {
    ...
    'uniswap/src': path.resolve(__dirname, '../../packages/uni-uniswap/src'),
  ...
  const exactAliases = [
    // Use web app-specific i18n entry that doesn't import wallet's i18n-setup (exact match only)
    {
      ...
      replacement: path.resolve(__dirname, '../../packages/uni-uniswap/src/i18n/index.web-app.ts'),
    },
  ]
  ...
  return {
    ...
    plugins: [
      ...
      // Cloudflare plugin removed for local development
