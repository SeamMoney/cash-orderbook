...
- For reruns of visual-only chart assertions (e.g., `VAL-CHART-005`) when chart data is empty/rate-limited, validate computed wrapper/container styles (`border`, `background`, `padding`) and class chain evidence instead of relying on live chart candles; avoid rapid polling loops.
- If `agent-browser errors` intermittently emits repeated parse noise (e.g., `Unexpected token "P"`), prefer `agent-browser console` output and computed-style checks as the primary evidence stream.
